...
const OnboardingPage = () => {
  ...
  return (
    <div className='p-5  justify-items-center align-items-center'>
      <div className='w-[40vw] rounded-[1vw] border p-5 border-secondary border-opacity-40  justify-items-center align-items-center flex-col '>
        <div className='text-2xl font-bold mb-5'>Update Your Profile</div>
        <div className='' >{onBoardingData?.profilePic ? <img className='size-16 rounded-full mb-2' src={onBoardingData?.profilePic} alt="Photo Preview" /> : <CameraIcon className='size-10  rounded-full mb-2'/>}</div>
        <div><button onClick={generateAvatar} className='px-5 py-1 mb-1 bg-secondary text-xs font-semibold w-fit rounded-full text-black'><Shuffle className='size-4 inline mr-2'/> Generate Random Avatar</button></div>
        ...
        <form className='w-full' onSubmit={handleSubmit}>
          ...
          <MapPin className='size-4 absolute -translate-y-6 translate-x-3'/>
              ...
              <div className='flex gap-5'>
                  <Link to="/" className='mt-5  rounded-full flex w-full justify-center items-center text-md py-1.5 font-semibold text-black bg-red-600'>
                    Cancel
                  </Link>

                  <button className='mt-5  rounded-full w-full   text-md py-1.5 font-semibold text-black bg-primary' disabled={isPending} type="submit"> { isPending ? (<> <LoaderIcon className='animate-spin inline size-5'/> Updating...</> ) : (<><GlobeIcon className='size-5 inline mr-1'/> Update Details</> ) } </button>
